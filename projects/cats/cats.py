...
def pick(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    # return [paragraphs[i] for i in range(len(paragraphs)) if select(paragraphs[i])][k]
    selected, unselected = [],[]
...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def func(str):
        str = lower(remove_punctuation(str))
        strs = split(str)
        for str1 in strs:
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    #     if diff_function(typed_word, word)<=limit:
    #         results.append()
    # END PROBLEM 5
...
def feline_fixes(typed, source, limit):
    ...
    if len(typed) == 0 and len(source) == 0:
        ...
        return 1 + feline_fixes('', source[1:], limit)
    elif len(typed) != 0 and len(source) == 0:
